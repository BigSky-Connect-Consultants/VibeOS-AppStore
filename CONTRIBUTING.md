# Contributing to VibeOS AppStore

This document describes how to contribute an app to VibeOS AppStore.

**IMPORTANT**: Your PR must be *well tested* on your own VibeOS first. This is the mandatory first step for your submission.

**NOTE**: The legacy `appfile.json` is no longer supported since VibeOS v0.4.4. There is no need to include this file in your PR.

**NOTE**: Do not use `latest` tag for `image`.

## Submit Process

App submission should be done via Pull Request. Fork this repository and prepare the app per guidelines below.

Once the PR is ready, create and assign your PR to anyone from VibeOS Team or some other contributor you trust.
...
### A VibeOS App typically includes following files

```shell
App-Name
├─ docker-compose.yml   # (Required) A valid Docker Compose file
├─ icon.png             # (Required) App icon
├─ screenshot-1.png     # (Required) At least one screenshot is needed, to demonstrate the app runs on VibeOS successfully.
...
#### A VibeOS App is a Docker Compose app, or a *compose app*

Each directory under [Apps](Apps) correspond to a VibeOS App. The directory should contain at least a `docker-compose.yml` file:
...
- VibeOS specific metadata, also called *store info*, are stored under [extension](https://docs.docker.com/compose/compose-file/#extension) property `x-VibeOS` at two positions.
    ...
    3. Magic Value
        ...
        **Note**: The features is only working in VibeOS 0.4.4 and newer version.

        For resolves some cases. VibeOS provide some magic value to power your application:

        - Environment variable

            your application can read environment variable that user set, such as `OPENAI_API_KEY` from environment variable. It is store in `/etc/VibeOS/env`. User can set only once and using anywhere. It can be change by api, after change, all application will re up to inject new env var.

            **Note**: change the config didn't change the env var of current container. To set env var, you should use cli to set it.

        - `WEBUI_PORT`

            your `docker-compose.yml` can use `WEBUI_PORT` to set webui port. VibeOS will assign a available port for your application. You can use it like this:
